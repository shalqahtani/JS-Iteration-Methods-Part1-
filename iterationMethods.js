...
const squared = Myarray.map(function (num) {
  return num ** 2;
});
console.log(squared);

//task2-part(4)
const multipliedBytwo = Myarray.map(function (num) {
  return num * 2;
});
...
const greaterOrEqual20 = newArray.filter(function (num) {
  if (num >= 20) {
    return true;
  } else {
    return false;
...
//task3-(6)
const divisiblebyfive = newArray.filter(function (item) {
  if (item % 5 == 0) {
    return true;
  } else {
    return false;
